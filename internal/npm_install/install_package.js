...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        yield downloadAndVerifyPackage(URL, INTEGRITY);
        const pkg = parsePackage();
        createBazelFiles(pkg);
    });
}
exports.main = main;
function downloadAndVerifyPackage(url, integrity) {
    return __awaiter(this, void 0, void 0, function* () {
...
`;
    ...
    writeFileSync(path.posix.join(".", "BUILD.bazel"), BUILD_FILE_HEADER + buildFile);
}
function mkdirp(p) {
    if (!fs.existsSync(p)) {
...
function printPackage(pkg) {
    ...
    const pkgFilesStarlark = pkg._runfiles.length
        ...
        : "";
    const notPkgFiles = pkg._files.filter((f) => !pkg._runfiles.includes(f));
    ...
    const dtsStarlark = dtsSources.length
        ? starlarkFiles("srcs", dtsSources, `# ${PACKAGE} package declaration files`)
        : "";
...
function resolveMainFile(pkg, mainFileName) {
    ...
    if (mainEntryField) {
        ...
        else if (typeof mainEntryField === "object" &&
